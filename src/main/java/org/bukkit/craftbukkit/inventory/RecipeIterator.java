...
//        this.smeltingVanilla = net.minecraft.item.crafting.FurnaceRecipes.smelting().smeltingList.keySet().iterator();
		// TODO
		this.smeltingCustom = Collections.<net.minecraft.item.ItemStack>emptyList().iterator();
		this.smeltingVanilla = net.minecraft.item.crafting.FurnaceRecipes.smelting().getSmeltingList().keySet().iterator();
	}
	...
	public Recipe next()
	{
		...
		{
			removeFrom = recipes;
			if (recipes.next() instanceof ShapelessRecipe)
				return ((IMixinShapelessRecipes) recipes.next()).toBukkitRecipe();
			else if (recipes.next() instanceof ShapedRecipe)
				return ((IMixinShapedRecipes) recipes.next()).toBukkitRecipe();
			return new CustomModRecipe(recipes.next());
		}
		else
		{
